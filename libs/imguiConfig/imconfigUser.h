...
// ImDrawCmd* cmd, void* my_renderer_user_data);
...
// Have the Item Picker break in the ItemAdd() function instead of
// ItemHoverable() - which is earlier in the code, will catch a few extra items,
// allow picking items other than Hovered one. This adds a small runtime cost
...
#define IM_VEC2_CLASS_EXTRA                                                    \
    ImVec2(const Blob::Vec2<> &f) {                                            \
        x = f.x;                                                               \
        y = f.y;                                                               \
    }                                                                          \
    operator Blob::Vec2<>() const { return Blob::Vec2<>(x, y); }
/*
#define IM_ASSERT(expr)							\
     (static_cast <bool> (expr)						\
      ? void (0)							\
      : throw ::Blob::Exception (std::string(#expr) + std::string(__FILE__) +
