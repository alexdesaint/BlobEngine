...
namespace Blob::GL {
	...
	/*void Renderable::loadObjFile(const std::string &file) {
			...
			if (m[1].compare("v") == 0) {
				...
				for (int i = 0; i < 3; i++) {
					if ((vp[i] - 1 < 0) || (vp[i] - 1 >= countV))
						throw Exception("ERROR: invalid vertex position index in face");

					if ((vt[i] - 1 < 0) || (vt[i] - 1 >= countVt))
						throw Exception("ERROR: invalid texture coord index in face");

					if ((vn[i] - 1 < 0) || (vn[i] - 1 >= countVn))
						throw Exception("ERROR: invalid vertex normal index in face");
		...
		if (countV != sizeV)
			throw Exception(".obj parser : Error on reading V");

		if (countVt != sizeVt)
			throw Exception(".obj parser : Error on reading Vt");

		if (countVn != sizeVn)
			throw Exception(".obj parser : Error on reading Vn");

		if (countF != sizeF)
			throw Exception(".obj parser : Error on reading F");
	...
	Renderable::Renderable(const std::string &file) {
		...
		if (extention == "obj") {
			...
			throw Exception(std::string("Cannot open the file extention : *.") + extention);
		}
	}
