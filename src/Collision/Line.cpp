...
namespace Blob::Collision {
    ...
    bool Line::overlap(const Rectangle &rectangle) const {
        Point p1{rectangle.position.operator+({rectangle.size.x / 2, rectangle.size.y / 2})};
        Point p2{rectangle.position.operator+({-rectangle.size.x / 2, rectangle.size.y / 2})};
        Point p3{rectangle.position.operator+({-rectangle.size.x / 2, -rectangle.size.y / 2})};
        Point p4{rectangle.position.operator+({rectangle.size.x / 2, -rectangle.size.y / 2})};

        if (rectangle.overlap(positionA) || rectangle.overlap(positionB) || overlap(Line(p1, p2)) || overlap(Line(p2, p3)) || overlap(Line(p3, p4)) || overlap(Line(p4, p1)))
            return true;
        return false;
    }
...
std::unordered_set<Maths::Vec2<int32_t>> Line::rasterize() const {
