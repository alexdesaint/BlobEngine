...
Mesh::Mesh(GL::VertexArrayObject &vertexArrayObject, const Material::Material &material)
    : material(&material), vertexArrayObject(vertexArrayObject) {}
...
template<>
void Mesh::setIndices(uint8_t *indices, int32_t numOfIndices) {
    setIndices(indices, numOfIndices, GL::Core::getType<uint8_t>());
}

template<>
void Mesh::setIndices(uint16_t *indices, int32_t numOfIndices) {
    setIndices(indices, numOfIndices, GL::Core::getType<uint16_t>());
}

template<>
void Mesh::setIndices(uint32_t *indices, int32_t numOfIndices) {
