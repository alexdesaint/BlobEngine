...
namespace BlobEngine::BlobGL::Text {
	...
	void createVBO() {
		...
		for (i = 0; i < numberOfLetter; i++) {
			...
			data[i * 4] 	= {{0.f, 	0.f}, {x,			y + 7.f/256}}; //bg
			data[i * 4 + 1] = {{w/10,   0.f}, {x + w/32, 	y + 7.f/256}};    //bd
			data[i * 4 + 2] = {{w/10,   1.f}, {x + w/32, 	y}};    //hd
			data[i * 4 + 3] = {{0.f, 	1.f}, {x,   		y}};    //hg
		}

		data[i * 4] 	= {{0.f, 0.f}, {0.f, 1.f}}; //bg
		data[i * 4 + 1] = {{1.f, 0.f}, {1.f, 1.f}}; //bd
		data[i * 4 + 2] = {{1.f, 1.f}, {1.f, 0.f}}; //hd
		data[i * 4 + 3] = {{0.f, 1.f}, {0.F, 0.f}}; //hg
