...
public:
    ...
    void setBuffer(const VertexBufferObject &vbo,
                   int32_t stride,
                   uint32_t dataOffset = 0,
                   uint32_t bufferPosition = 0,
                   uint32_t divisor = 0) const;

    void setIndicesBuffer(const VertexBufferObject &vbo) const;

    void setArray(uint32_t numValuePerArray,
                  uint32_t outPosition,
                  uint32_t dataType,
                  uint32_t relativeOffset,
                  bool normalized = false,
                  uint32_t bufferPosition = 0) const;
...
template<>
void VertexArrayObject::setArray<float>(uint32_t numValuePerArray,
                                        uint32_t outPosition,
                                        uint32_t relativeOffset,
                                        bool normalized,
