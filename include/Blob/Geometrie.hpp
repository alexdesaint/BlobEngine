...
public:
    ...
    // operator with T
    Vec2 operator+(T a) const { return {a + x, a + y}; }

    Vec2 operator-(T a) const { return {x - a, y - a}; }
    ...
    // Functions
    [[nodiscard]] T length2() const { return x * x + y * y; }

    [[nodiscard]] float length() const;

    [[nodiscard]] T scalaire(Vec2 B) const { return x * B.x + y * B.y; }
    ...
    [[nodiscard]] Vec2 rotate() const { return {-y, x}; }
...
public:
    ...
    Vec2f getVector() { return {pointB.x - pointA.x, pointB.y - pointA.y}; }

    double Length2() { return (pointB.x - pointA.x) * (pointB.x - pointA.x) + (pointB.y - pointA.y) * (pointB.y - pointA.y); }
    ...
    float getConstant() { return pointA.y - getGradient() * pointA.x; }
};
